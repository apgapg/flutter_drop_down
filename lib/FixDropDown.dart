...
const double _kMenuItemHeight = 56.0;
...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  final List<FixDropdownMenuItem<T>> items;
...
/// An item in a menu created by a [FixDropDown].
///
/// The type `T` is the type of the value the entry represents. All the entries
/// in a given menu must represent values with consistent types.
class FixDropdownMenuItem<T> extends StatelessWidget {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const FixDropdownMenuItem({
    Key key,
    this.value,
    @required this.child,
  })  : assert(child != null),
  ...
  ///
  /// Eventually returned in a call to [FixDropDown.onChanged].
  final T value;
...
/// An inherited widget that causes any descendant [FixDropDown]
/// widgets to not include their regular underline.
///
/// This is used by [DataTable] to remove the underline from any
/// [FixDropDown] widgets placed within material data tables, as
...
class DropdownButtonHideUnderline extends InheritedWidget {
  ...
  /// Returns whether the underline of [FixDropDown] widgets should
  /// be hidden.
...
/// Typically, an enum is used. Each [FixDropdownMenuItem] in [items] must be
...
/// See also:
///
///  * [FixDropdownMenuItem], the class used to represent the [items].
...
class FixDropDown<T> extends StatefulWidget {
  ...
  FixDropDown({
    ...
    this.isDense = false,
        ...
        assert(value == null || items.where((FixDropdownMenuItem<T> item) => item.value == value).length == 1),
        super(key: key);
...
class _DropdownButtonState<T> extends State<FixDropDown<T>> with WidgetsBindingObserver {
  int _selectedIndex;
  ...
  @override
  void didUpdateWidget(FixDropDown<T> oldWidget) {
    super.didUpdateWidget(oldWidget);
    _updateSelectedIndex();
  }

  void _updateSelectedIndex() {
    assert(widget.value == null || widget.items.where((FixDropdownMenuItem<T> item) => item.value == widget.value).length == 1);
    _selectedIndex = null;
